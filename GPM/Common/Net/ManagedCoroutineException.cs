            ...
            for (int i = 0; i < coroutine.stackTrace.GetFrames().Length - hideStack; i++)
            {
                ...
                if (method != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            filePath = filePath.Substring((projectPath.Length + 1), filePath.Length - (projectPath.Length + 1));
                        }

                        int line = frame.GetFileLineNumber();
                        stackText += string.Format("{0}:{1}() - ( {2} : {3} )", method.DeclaringType, method.Name, filePath, line);
                    }
